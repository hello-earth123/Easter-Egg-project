...
export default class Valcony extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.safeSpawnPoints = [[490, 600], [400, 210], [300, 400], [400, 800]];
    ...
    preload() {
        this.load.image("Valcony", "/static/assets/map/balcony.png");
        this.load.tilemapTiledJSON('ValconyTile', '/static/assets/map/balcony.json');
        // BGM
        this.load.audio("bgm_valcony", "/static/assets/sound/background/bgm_valcony.wav");
    ...
    create() {
        ...
        // 1. 씬 BGM
        this.SoundManager.playBgm("bgm_valcony")
        ...
        const map = this.add.image(0, 0, "Valcony").setOrigin(0);
        const tile = this.make.tilemap({ key: 'ValconyTile' });
        ...
        // 게임 시작 자동 컷씬 스크립트
        const introScript = [
            { cmd: "say", text: "이프리트: (성에 들어오니 달이 붉은색으로 바뀌었다고…?!)." },
            { cmd: "say", text: "프라가라흐: 어때… 붉은 달… 아름답지 않나…." },
        ...
        this.time.delayedCall(500, () => {
            if ((this.playerStats.cutScene & 1 << 5) == 0) {
                this.cutscene.play(introScript);
                this.playerStats.cutScene += (1 << 5);
            }
            else {
