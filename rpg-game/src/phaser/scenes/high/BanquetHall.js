...
export default class BanquetHall extends Phaser.Scene {
    ...
    init(data) {
        ...
        const portalSpawnPoints = {
            east: { x: 150, y: 600 },   // Scene의 east 포탈을 타면 여기서 등장
            south: { x: 800, y: 200 },
            west: { x: 1450, y: 910 },
            north: { x: 800, y: 910},
        };
    ...
    constructor() {
        super({ key: "BanquetHall" });

        this.mapKey = "BanquetHall";

        this.mapName = "연회장";      // 맵 이름
    ...
    preload() {
        this.load.image("banquet_hall", "/static/assets/map/banquet_hall.png");
        this.load.tilemapTiledJSON('banquet_hallTile', '/static/assets/map/banquet_hall.json');
        // BGM
    ...
    create() {
        ...
        const map = this.add.image(0, 0, "banquet_hall").setOrigin(0);
        const tile = this.make.tilemap({key: 'banquet_hallTile'});
        ...
        // 포탈 4개 생성
        this.portals = {
            east:  this.physics.add.sprite(1450, 910, "portal"),
            west:  this.physics.add.sprite(150, 600, "portal"),
