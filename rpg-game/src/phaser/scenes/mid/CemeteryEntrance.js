...
export default class CemeteryEntrance extends Phaser.Scene {
    ...
    constructor() {
        super({ key: "CemeteryEntrance" });

        this.mapKey = "CemeteryEntrance";

        this.mapName = "음산한 무덤가"; // 맵 이름
    ...
    preload() {
        this.load.image("cemetery_entrance", "/static/assets/map/cemetery_entrance.png");
        this.load.tilemapTiledJSON('cemetery_entranceTile', '/static/assets/map/cemetery_entrance.json');
        // BGM
    ...
    create() {
        ...
        const map = this.add.image(0, 0, "cemetery_entrance").setOrigin(0);
        const tile = this.make.tilemap({ key: 'cemetery_entranceTile' });
        ...
        this.portals = {
            ...
            south: this.physics.add.sprite(855, 1100, "portal"),
            north: this.physics.add.sprite(855, 300, "portal")
        };
        ...
        // 게임 시작 자동 컷씬 스크립트
        const introScript = [
            { cmd: "say", text: "이프리트: 마법 도시 근처에 이렇게나 큰 건축물이 있는데 아무도 몰랐다고…?" },
        ...
        this.time.delayedCall(500, () => {
            if ((this.playerStats.cutScene & 1 << 3) == 0) {
                this.cutscene.play(introScript);
                this.playerStats.cutScene += (1 << 3);
            }
            else {
