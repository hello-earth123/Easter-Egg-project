...
export default class Road1 extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.safeSpawnPoints = [[300, 330], [500, 470], [700, 290], [900, 500], [800, 600]];
    ...
    preload() {
        this.load.image("road1", "/static/assets/map/road1.png");
        this.load.tilemapTiledJSON('road1Tile', '/static/assets/map/road1.json');
        // BGM
        this.load.audio("bgm_road", "/static/assets/sound/background/bgm_road.wav");
    ...
    create() {
        ...
        const map = this.add.image(0, 0, "road1").setOrigin(0);
        const tile = this.make.tilemap({ key: 'road1Tile' });
        ...
        // 게임 시작 자동 컷씬 스크립트
        const introScript = [
            { cmd: "say", text: "프라가라흐: 어이 꼬맹이. 아랫쪽 포탈에서 기묘한 힘이 느껴진다…." },
            { cmd: "say", text: "(아랫쪽 포탈로 가보자)." },
        ...
        this.time.delayedCall(500, () => {
            if ((this.playerStats.cutScene & 1 << 1) == 0) {
                this.cutscene.play(introScript);
                this.playerStats.cutScene += (1 << 1);
            }
            else {
