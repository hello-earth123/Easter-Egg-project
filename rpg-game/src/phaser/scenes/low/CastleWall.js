...
export default class CastleWall extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.safeSpawnPoints = [[400, 300], [1200, 900], [400, 900], [1200, 300], [800, 600]];
    ...
    create() {
        ...
        const map = this.add.image(0, 0, "castle_wall").setOrigin(0);
        const tile = this.make.tilemap({ key: 'castle_wallTile' });
        ...
        // 게임 시작 자동 컷씬 스크립트
        const introScript = [
            { cmd: "say", text: "내 이름은 이프리트. 뛰어난 마법사가 되고자 한 평범한 소녀다.." },

            { cmd: "say", text: "하지만 현실은… 생각보다 잔혹했다.." },
            { cmd: "say", text: "도시 학교의 마법사들은 나를 비웃었고, 제대로 상대해 주지도 않았다.." },
            { cmd: "wait", time: 400 },

            { cmd: "say", text: "“그따위 실력으로 마법사를 꿈꾼다고? ㅋㅋㅋㅋ” 라는 말은 하루에도 열 번 넘게 들었다.." },
            ...
            { cmd: "say", text: "…." },
            { cmd: "say", text: "…잠깐. 방금 스태프에서 목소리가 들린거 같은데…?." },

            { cmd: "say", text: "???: 드디어… 드디어 나를 깨워주는군.." },
            { cmd: "say", text: "이프리트: !? 뭐, 뭐야!? 누… 누구야!?." },

            { cmd: "say", text: "???: 나는 프라가라흐. 봉인된 지 천 년, 나를 깨운 자여…." },
            { cmd: "say", text: "프라가라흐: 내 봉인을 풀어준다면… 너에게 강력한 힘을 주겠다.." },

            { cmd: "say", text: "이프리트: 강력한… 힘…?." },
            { cmd: "wait", time: 400 },

            { cmd: "say", text: "그 순간, 스태프가 희미하게 웃은 것 같았다.." },
            { cmd: "say", text: "프라가라흐: 자, 소녀여. 모험을 시작하자.." },
            ...
            { cmd: "say", text: "프라가라흐: 모든 것이 바뀔 것이다…." },
            { cmd: "say", text: "프라가라흐: 몬스터를 잡아 그 힘을 탈취해 나의 봉인을 풀면, 그 힘의 파편이 너에게도 깃들거야.." },

            { cmd: "say", text: "프라가라흐: i를 눌러 인벤토리 창, p를 눌러 너와 나의 능력치 창, k를 눌러 마법 스킬창을 열 수 있다.." },
            { cmd: "say", text: "프라가라흐: 그리고 단축키 슬롯에 스킬과 아이템을 드래그 앤 드롭하여 단축키로 사용할 수 있지.." },
            { cmd: "say", text: "프라가라흐: esc를 눌러 주기적으로 모험 내용을 작성하는 것도 잊지말라고.." },
